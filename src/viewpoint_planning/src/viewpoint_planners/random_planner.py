...
class RandomPlanner(PlannerEvalMixin):
    ...
    Strategy (uniform random neighbour sampling) is UNCHANGED. Only the
    environment (grid_size, voxel_size) and the evaluation code
    (calculate_F1 / sigma / occluded recall via PlannerEvalMixin) are aligned
    with RH-NBV for a fair comparison. mesh_coordinates/mesh_tree are now
    required so reconstruction F1 can be scored like the other planners.
    """
    ...
    def __init__(
        ...
        # Evaluation parity with RH-NBV (mesh GT + shared metric state).
