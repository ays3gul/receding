#!/usr/bin/env python3
"""
test_comparison_node_integrated_existing_plots.py
=================================================
Runs RH-NBV and GradientNBV under identical conditions, prints a compact
comparison table, saves JSON/CSV results, and reuses the existing plotting
helpers in the project:

- plot_coverage_progression
- plot_3d_trajectory
- plot_reconstruction_comparison
- plot_candidate_sequences / plot_candidate_sequences_grid

Usage:
    python test_comparison_node.py

Expected outputs:
    results/results_rh_nbv_<occ>.json
    results/results_gradientnbv_<occ>.json
    results/comparison_live_summary.csv
    results/plots/*.png
"""
...
def save_existing_style_plots(rh_vp, grad_vp, rh_rows, grad_rows, occ):
    ...
    # 1) Existing coverage plot style.
    ...
    # 2) Additional useful comparison plots, simple style.
    simple_specs = [
    ...
    # 3) Existing 3D trajectory style: one image per method.
    ...
    # 4) Existing reconstruction style: one image per method.
    ...
    # 5) Existing RH candidate sequence visualization.
    ...
    # 6) Compact normalized final summary.
