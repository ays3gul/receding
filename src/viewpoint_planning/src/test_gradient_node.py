...
# Shared single-source-of-truth parameters identical to the RH-NBV planner
# (grid size, target, reach bounds, start jitter). Same import path fallback
# as GradientNBVPlanner above.
try:
...
# Target node on the bunny body, camera-facing and observable. Pulled from the
# shared config so it is byte-identical to viewpoint_planning.py.
...
def spawn_occlusion(sdf, occ):
    # 4 scenarios, IDENTICAL to viewpoint_planning.py, chosen so the RH-vs-
    # myopic gap widens as camera freedom shrinks (frontal < half_box < tunnel).
    if occ == "none":
        pass
    elif occ == "frontal":
...
def run_single_trial(trial_idx, occ, run_dir, mesh_coords, mesh_tree,
    ...
    # Per-trial start variation, IDENTICAL to RH-NBV: trial 0 deterministic,
    # later trials get the same reproducible +/-3cm jitter (same seed =
    # BASE_SEED + trial_idx). This pairs the two planners trial-for-trial and
    # gives them matched start variance — without it GradientNBV had zero
    # variance while RH had real variance, making the std comparison meaningless.
    ...
    planner = GradientNBVPlanner(
        ...
        # Shared reconstruction volume — identical to RH-NBV (was a stale
        # [0.3,0.3,0.7] here, which differed from RH's [0.3,0.6,0.3] and broke
        # the comparison). Now both pull GRID_SIZE from fair_comparison_config.
    ...
    tp = [0]; fp = [0]; fn = [0]
    sigmas = [0.0]; occ_recalls = [0.0]   # Burusa Table II parity with RH-NBV
    recon_snapshots = []                   # per-iteration reconstruction
    ...
    for i in range(NUM_ITERS):
        ...
        if ok:
            ...
            # Record the occluded-mesh baseline after the first real view,
            # exactly as RH does (occluded recall is measured relative to what
            # was hidden at the starting view).
        ...
        # Same tunnel-panel voxel masking RH applies, so panel surfaces aren't
        # counted as F1 false positives. Only the tunnel scenario's panels
        # intrude into the ROI clip cube; all other occluders sit outside it.
        occ_positions = None
        if occ == "tunnel":
            occ_positions = [
        ...
        # Burusa Table II metrics (same helpers as RH-NBV).
        sigmas.append(planner.compute_sigma())
        occ_recalls.append(planner.compute_occluded_recall())
        # Snapshot the cumulative target reconstruction for the per-iteration
        # evolution plot (same convention as RH-NBV).
    ...
    # --- Per-iteration reconstruction evolution (supervisor request) ---
    if recon_snapshots:
